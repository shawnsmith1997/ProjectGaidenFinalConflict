...
case 0:
    ...
    CreateBattleDialogueBasic(WordWrap(string(Attacker.Character.Name) +' uses the ' + 'Mobility Pimento' + '!^@ ',200,'#',false), self.id);
...
case 50:
    ...
    Reciever.Character.MoveRange += Gain;//Damage wont go over maxHP, no need to test a second time
    HighestZ = 25;
    EXP += Calculate_EXP(25,round(Reciever.Character.MoveRange*0.25),Reciever.Character.MaxHealth,10,false)//Z,D,M,Min,Bonus
    with(Reciever){
...
//===================
case 60:
    CreateBattleDialogueBasic(WordWrap(string(Reciever.Character.Name) + ' Move Range increased by ' + string(round(Gain)) + '!',200,'#',false), self.id);
