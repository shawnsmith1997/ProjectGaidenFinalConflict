...
Action = argument2;
//Spell = 'Blaze';//argument3;
//SpellLevel = 1;//argument4;

instance_create(0,0,BattleCutsceneControler);
BattleCutsceneControler.Spell = 'LaserEye';//what animation to play
//BattleCutsceneControler.SpellLevel = 1;
...
//Find all targets.
//FindAttackRange(instance_position(round(CurrentTarget.x), round(CurrentTarget.y), objCell),Get_Spell_Stats(CurrentCharacter.Spells[Spell],SpellLevel,4)-1,-1,FixSpellTarget(Get_Spell_Stats(CurrentCharacter.Spells[Spell],SpellLevel,6),CurrentCharacter.GuyType));
BattleCutsceneControler.NumberOfTargets = Cursor.NumberOfTargets;//calculate number of targets (already calculated via target selection since we hit all units)
...
if(Action = 'LaserEye'){
    ...
    BattleCutsceneControler.Action = 'FireLaserEye';
