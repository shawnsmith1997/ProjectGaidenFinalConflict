...
case 0:
    ...
    CreateBattleDialogueBasic(WordWrap(string(Attacker.Character.Name) +' uses the ' + 'Power Wine' + '!^@ ',200,'#',false), self.id);
...
case 50:
    ...
    Reciever.Character.Attack += round(Gain);//Damage wont go over maxHP, no need to test a second time
    HighestZ = 25;
    EXP += Calculate_EXP(25,round(Reciever.Character.Attack*0.25),Reciever.Character.MaxHealth,10,false)//Z,D,M,Min,Bonus
    with(Reciever){
...
//===================
case 60:
    CreateBattleDialogueBasic(WordWrap(string(Reciever.Character.Name) + ' Attack increased by ' + string(round(Gain)) + '!',200,'#',false), self.id);
